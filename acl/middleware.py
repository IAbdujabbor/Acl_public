...
# Optional: Add local dev origins
ALLOWED_DEV_ORIGINS = [
    "http://localhost:8000",
    "http://127.0.0.1:8000"
]
...
class AllowOnlyMySitesMiddleware:
    ...
    def __call__(self, request):
        ...
        # ✅ Allow in development mode (DEBUG = True or via env variable)
        if os.getenv("DJANGO_ALLOW_ALL", "false") == "true":
            return self.get_response(request)

        allowed_origins = ALLOWED_IFRAME_ORIGINS + ALLOWED_DEV_ORIGINS

        # ✅ If referer is set and not from an allowed domain
        ...
        # ✅ If origin is set and not from an allowed domain
...
class CSPMiddleware:
    ...
    def __call__(self, request):
        ...
        response["Content-Security-Policy"] = (
            "frame-ancestors https://solwindenergy.com https://jafaenergy.com http://localhost:8000 http://127.0.0.1:8000"
        )
        return response
